...
static void bond_manage_bond_complete_callback(bond_init_info_t *info,bond_manage_evt_param_t *evt,uint8_t status);
static void bond_manage_bond_init_start(bond_init_info_t *info);
...
QUEUED_ASYNC_PROCESS_COMMON_CALLBACK(bond_process,
    BOND_MG_MAX_CONNECT_NUM, bond_init_info_t,
    bond_manage_bond_init_start, bond_manage_bond_complete_callback);
    ...
    if(p_cfg !=NULL)
    {
        if(bond_manage_gapc_pairing_params_verify(&p_cfg->pairing_feat))
        {
            ...
            if( (l_bond.cfg.pair_mode != BOND_MG_PAIR_MODE_NO_PAIR)
                &&( (l_bond.cfg.pairing_feat.auth&GAP_AUTH_BOND) == GAP_AUTH_BOND)
              )
            {
                bond_save_init();
            }
            //3. init connect role
            //bond_manage_connect_role_parse_init();
            //4.init bond id ->connect index
            for(uint8_t i=0;i<BOND_MG_MAX_CONNECT_NUM;i++)
            {
                memset((uint8_t *)&l_bond.dev_info[i],0,sizeof(bond_manage_dev_info_t));
                l_bond.dev_info[i].id = BOND_MG_INVALID_ID;
            }
...
{
    uint8_t conn_idx = KE_IDX_GET(src_id);
    //clear connect index to bond id
    if(param->reason == LL_ERR_TERMINATED_MIC_FAILURE)
    {
        //bond info lost,delete local data.
        bond_node_id_t id = bond_manage_connect_index_to_bond_id(conn_idx);

        if( (id != BOND_MG_INVALID_ID)
            &&( l_bond.cfg.pair_mode != BOND_MG_PAIR_MODE_NO_PAIR)
            &&( (l_bond.cfg.pairing_feat.auth&GAP_AUTH_BOND) == GAP_AUTH_BOND)
          )
        {
            ble_bond_err_t err = bond_save_delete_node(id);

            if(err!=BLE_BOND_SUCCESS)
            {
                LOG(LOG_LVL_ERROR,"bond_save_delete_node conhdl %d node id %d!!!\n",param->conhdl,id);
            }
        }
    }
    ...
    {
        //reset
        memset((uint8_t *)&l_bond.dev_info[conn_idx],0,sizeof(bond_manage_dev_info_t));
        l_bond.dev_info[param->conhdl].id = BOND_MG_INVALID_ID;
    }
}
...
static void osapp_gapc_cmp_evt_handler(ke_msg_id_t const msgid, struct gapc_cmp_evt const *param,ke_task_id_t const dest_id,ke_task_id_t const src_id)
{
    LOG(LOG_LVL_INFO,"bond mamage gapc cmp evt op=%d\n", param->operation);
...
static void osapp_gapc_security_ind_handler(ke_msg_id_t const msgid, struct gapc_security_ind const *param,ke_task_id_t const dest_id,ke_task_id_t const src_id)
{
    ...
    if(conn_idx>=BOND_MG_MAX_CONNECT_NUM)
    {
        LOG(LOG_LVL_ERROR,"bond mamage gapc_bond_req_ind_ conn_idx %d!!!\n",conn_idx);
        return;
    }
    else
    {
        if(l_bond.cfg.pair_mode !=BOND_MG_PAIR_MODE_NO_PAIR)
        {
            bond_init_info_t init = 
            {
                .auth = param->auth,
                .conn_idx = conn_idx,
            };
            queued_async_process_start(&bond_process,&init,NULL);
        }
    }
}
...
static void osapp_gapc_bond_req_ind_handler(ke_msg_id_t const msgid, struct gapc_bond_req_ind const *param,ke_task_id_t const dest_id,ke_task_id_t const src_id)
{
    ...
    switch(param->request)
    {
        ...
        case GAPC_LTK_EXCH:
        {
            ...
            {
                bond_manage_generate_ltk(p_ltk_local,param->data.key_size);
            }
        ...
        case GAPC_CSRK_EXCH:
        {
            ...
            if(l_bond.dev_info[conn_idx].id == BOND_MG_INVALID_ID)
            {
                bond_manage_generate_csrk(p_csrk_local);
            }
        ...
        case GAPC_PAIRING_FAILED:
            {
                ...
                queued_async_process_end(&bond_process,&evt, evt.pair_result.success);
            }
           break;
        default:
                                        ...
                                        ke_task_id_t const src_id)
{
    ...
    if(l_bond.cfg.evt)
    {
        ...
        l_bond.cfg.evt(BOND_MG_EVT_ENCRYPY_RESULT,&evt);
    }
}
...
void bond_manage_initiate_security_procedure(uint8_t conn_idx)
{
    ...
    }else
    {
        bond_init_info_t init = 
        {
            .auth = l_bond.cfg.pairing_feat.auth,
            .conn_idx = conn_idx,
        };
        queued_async_process_start(&bond_process,&init,NULL);
    }
...
void bond_manage_slave_req_security(uint8_t conn_index)
{
    ...
    cmd->auth = l_bond.cfg.pairing_feat.auth;//GAP_AUTH_REQ_NO_MITM_BOND;

    osapp_ahi_msg_send(cmd,sizeof(struct gapc_security_cmd),portMAX_DELAY);
}
/**
 ****************************************************************************************
 * @brief   Func bond_manage_gapm_start_connect_cmd [gap master role] send req msg to[gap slave role]
 *
 * @param[in] p_peers  The ponit of the gap_baddr array to  connect.
 * @param[in] num      The num bd_addr of the gap_baddr array.
 *
 * @return    The result of bond_manage_gapm_start_connect_cmd.
 ****************************************************************************************
 */
/*
ble_bond_err_t bond_manage_gapm_start_connect_cmd(struct gap_bdaddr *p_peers,uint8_t num)
{
    ble_bond_err_t err = BLE_BOND_SUCCESS;
    ...
    if(p_peers&&num)
    {
        ...
        key_len = sizeof(struct gap_bdaddr) * num;
        cmd = AHI_MSG_ALLOC_DYN(GAPM_START_CONNECTION_CMD,TASK_ID_GAPM,gapm_start_connection_cmd,key_len);

        //todo: other params
        cmd->op.code = GAPM_CONNECTION_GENERAL;
        cmd->op.addr_src = GAPM_STATIC_ADDR;
        cmd->op.state = 0;

        cmd->nb_peers = num;
        memcpy((uint8_t *)cmd->peers,(uint8_t *)p_peers,key_len);
        //add to cnetral array
        for(uint8_t i=0;i<num;i++)
        {
            bond_manage_add_role_to_central_array(p_peers[i].addr);
        }

        osapp_msg_build_send(cmd,(sizeof(struct gapm_start_connection_cmd)+key_len));
    }
    else
    {
        err = BLE_BOND_ERROR_INVALID_PARAM;
...
static void bond_manage_bond_init_start(bond_init_info_t *info)
{
    if(l_bond.dev_info[info->conn_idx].role == GAP_ROLE_PERIPHERAL)
    {
        bond_manage_pair_response(info->conn_idx);
    }else
    {
        bond_manage_gapc_bond_cmd(info->conn_idx);
    }
}
...
static void bond_manage_gapc_bond_cmd(uint8_t conn_idx)
{
    ...
    osapp_ahi_msg_send(cmd, sizeof(struct gapc_bond_cmd),portMAX_DELAY);
}
...
static void bond_manage_bond_process_connect(uint8_t conn_idx)
{
    ...
    {
        //no pair or no bond,don't find info in flash.
        bond_save_recover_complete(BOND_MG_NO_BOND,BOND_MG_INVALID_ID);
    }
    else
    {
...
static void bond_manage_pair_result_to_save(uint8_t result,uint8_t conn_idx)
{
    ...
    if(conn_idx>=BOND_MG_MAX_CONNECT_NUM)
    {
        LOG(LOG_LVL_ERROR," bond_manage_pair_result_to_save conn_idx %d!!!\n",conn_idx);
        return;
    }
 ...
 ****************************************************************************************
 */
bool bond_manage_is_connected(uint16_t conn_idx)
{
    bool valid = false;
