 ...
 ****************************************************************************************
 */
#define NVDS_TAG_TABLE_MAX 53     // must be a prime number
...
static void nvds_erase_sectors(uint32_t base)
{
    uint8_t num = NVDS_DATA_SECTION_MAX_SIZE/NVDS_ERASE_SIZE;
    uint8_t i;
    for(i=0;i<num;++i)
    {
        nvds_env.erase(base);
        base += NVDS_ERASE_SIZE;
    }
}
...
static void nvds_move_to_another_blk()
{
    ...
    nvds_erase_sectors((uint32_t)new_base);
...
{
    return nvds_env.tag_table[idx].offset == 0 || nvds_env.tag_table[idx].name == name;
}
...
static bool nvds_tag_valid(uint8_t idx,uint8_t name)
{
    return nvds_env.tag_table[idx].name == name && nvds_env.tag_table[idx].offset != 0;
}
...
static nvds_tag_hash_table_t *nvds_tag_search(uint8_t tag_name)
{
    ...
    {
        if(nvds_tag_valid(hash,tag_name))
        {
            return &nvds_env.tag_table[hash];
        }else
        {
