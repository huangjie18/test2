...
{
#ifdef AES_128_ENC_HARD_ENABLE
#ifndef x86
	osapp_gapm_use_enc_blck_cmd(key,plaintext);
	if(pdTRUE!=xSemaphoreTake(ble_aec_enc_xSemaphore,10))
  {
      JMESH_LOGW(1,"aes128 encrytion","take semaphore failure\n");
  }
	aes_128_enc_last_result_get(ciphertext);
#endif
#else
    uint8_t w[176] ;
    key_expansion(key, w);
