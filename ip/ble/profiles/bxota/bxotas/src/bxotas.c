...
{
    uint8_t obsolete;
    flash_read(image_base + FEATURE_OFFSET_IN_IMAGE_HEADER,sizeof(obsolete),&obsolete);
    obsolete &= ~(1<<OBSOLETE_BIT_OFFSET_IN_FEATURE);
...
{
    uint8_t valid;
    flash_read(image_base + FEATURE_OFFSET_IN_IMAGE_HEADER,sizeof(valid),&valid);
    valid &= ~(1 << VALID_BIT_OFFSET_IN_FEATURE);
