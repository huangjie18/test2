...
void app_spi_init(periph_inst_handle_t hdl)
{
    ...
    spi_universal_func.pin_cfg_func(inst,param->mosi_pin_no,MOSI_PIN,true);
    spi_universal_func.pin_cfg_func(inst,param->miso_pin_no,MISO_PIN,true);
    if(inst->slave)
    {
        spi_universal_func.pin_cfg_func(inst,param->clk_pin_no,SPIS_CLK_PIN,true);
        spi_universal_func.pin_cfg_func(inst,param->u.slave.cs_pin_no,SPIS_CS_PIN,true);
    }else
    {
        spi_universal_func.pin_cfg_func(inst,param->clk_pin_no,SPIM_CLK_PIN,true);
        uint8_t i;
        for(i=0;i<SPIM_CS_NUM;++i)
        {
            if(1<<i & param->u.master.hw_cs_mask)
            {
                spi_universal_func.pin_cfg_func(inst,param->u.master.cs_pin_no[i],SPIM_CS0_PIN + i,true);
            }
        }
    ...
    periph_semaphore_init(&inst->env.spi_lock,1);
...
{
    app_spi_inst_t *inst = CONTAINER_OF(hdl, app_spi_inst_t, inst);
    if(periph_semaphore_p(&inst->env.spi_lock)==false)
    {
        return PERIPH_BUSY;
    }
...
static void spi_rx_done(app_spi_inst_t *inst)
{
    ...
    periph_semaphore_v(&inst->env.spi_lock);
...
periph_err_t app_spi_slave_operation_cancel(periph_inst_handle_t hdl,uint32_t *rx_remaining_length,uint32_t *tx_remaining_length)
{
    ...
    spi_universal_func.intr_op_func(inst,INTR_DISABLE);
    if(periph_semaphore_get_count(&inst->env.spi_lock))
    {
