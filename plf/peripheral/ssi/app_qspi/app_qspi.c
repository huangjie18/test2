...
N_XIP_SECTION void app_qspi_init(periph_inst_handle_t hdl)
{
    ...
    periph_semaphore_init(&inst->qspi_lock,1);
    qspi_universal_func.clk_gate_func(inst,CLR_CLK);
    qspi_universal_func.sys_stat_func(inst,QSPI_INIT);
}

N_XIP_SECTION void app_qspi_uninit(periph_inst_handle_t hdl)
{
    app_qspi_inst_t *inst = CONTAINER_OF(hdl, app_qspi_inst_t, inst);
    qspi_universal_func.clk_gate_func(inst,CLR_CLK);
...
N_XIP_SECTION periph_err_t app_qspi_std_write(periph_inst_handle_t hdl,uint8_t cs_sel_mask,uint8_t *data,uint32_t length)
{
    ...
    if(periph_semaphore_p(&inst->qspi_lock)==false)
    {
        return PERIPH_BUSY;
    }
    qspi_universal_func.clk_gate_func(inst,SET_CLK);
    ...
    qspi_universal_func.clk_gate_func(inst,CLR_CLK);
    periph_semaphore_v(&inst->qspi_lock);
...
N_XIP_SECTION periph_err_t app_qspi_multi_read_32bits(periph_inst_handle_t hdl,uint8_t cs_sel_mask,qspi_multi_read_32bits_param_t *param)
{
    app_qspi_inst_t *inst = CONTAINER_OF(hdl, app_qspi_inst_t, inst);
    if(periph_semaphore_p(&inst->qspi_lock)==false)
    {
    ...
    reg->CTRLR0 = FIELD_BUILD(SSI_SPI_FRF,param->dual_quad) | FIELD_BUILD(SSI_DFS,DFS_32_32_bits) | FIELD_BUILD(SSI_TMOD, Receive_Only) |
        FIELD_BUILD(SSI_SCPOL,Inactive_Low)|FIELD_BUILD(SSI_SCPH, SCLK_Toggle_In_Middle) | FIELD_BUILD(SSI_FRF, Motorola_SPI);
    reg->CTRLR1 = param->length - 1;
    reg->DMACR = 0;
    reg->SSIENR = SSI_Enabled;
    reg->DR = param->cmd;
    reg->DR = ADDR_EXTEND(param->addr_field.addr,param->addr_field.extend_byte);
    uint16_t fifo_depth = qspi_get_fifo_depth(inst);
    if(param->length > fifo_depth)
    {
        reg->DMARDLR = 7;
        reg->DMACR = FIELD_BUILD(SSI_RDMAE , Receive_DMA_Enabled);
        app_dmac_transfer_param_t dma_param;
        dma_param.src = (uint8_t *)&reg->DR_REVERSED;
        dma_param.dst = (uint8_t *)param->data;
        dma_param.length = param->length;
...
N_XIP_SECTION periph_err_t app_qspi_config_for_cache(periph_inst_handle_t hdl,uint8_t cs_sel_mask,qspi_multi_read_32bits_param_t *param)
{
    app_qspi_inst_t *inst = CONTAINER_OF(hdl, app_qspi_inst_t, inst);
    if(periph_semaphore_p(&inst->qspi_lock)==false)
    {
...
{
    app_qspi_inst_t *inst = CONTAINER_OF(hdl, app_qspi_inst_t, inst);
    if(periph_semaphore_get_count(&inst->qspi_lock))
    {
