...
static void uart_rx_complete(app_uart_inst_t *inst)
{
    ...
    if(BIT_FETCH(LSR,2)==Parity_Error)
    {
        LOG(LOG_LVL_ERROR,"comp parity error\n");
    ...
    {
        periph_semaphore_v(&inst->env.rx_lock);
...
static void uart_tx_complete(app_uart_inst_t *inst)
{
    ...
    periph_semaphore_v(&inst->env.tx_lock);
    // Call handler
...
static void uart_rec_error_isr(app_uart_inst_t *inst)
{
    ...
    if(BIT_FETCH(LSR,2)==Parity_Error)
    {
        LOG(LOG_LVL_ERROR,"uart parity error\n");
        inst->env.errordetect = true;
        //uart_rx_complete(inst);
    }
    if(BIT_FETCH(LSR,3)==Framing_Error)
...
void app_uart_init(periph_inst_handle_t hdl)
{
    ...
    uart_universal_func.pin_cfg_func(inst,param->rx_pin_no,UART_RXD_PIN,true);
    uart_universal_func.pin_cfg_func(inst,param->tx_pin_no,UART_TXD_PIN,true);
    reg_uart_t *reg = inst->reg;
    if(param->flow_control_en)
    {
        uart_universal_func.pin_cfg_func(inst,param->rts_pin_no,UART_RTS_PIN,true);
        uart_universal_func.pin_cfg_func(inst,param->cts_pin_no,UART_CTS_PIN,true);
    }
    ...
    periph_semaphore_init(&inst->env.tx_lock,1);
    periph_semaphore_init(&inst->env.rx_lock,1);
...
periph_err_t app_uart_read(periph_inst_handle_t hdl,uint8_t *bufptr, uint32_t size, void (*callback) (void*,uint8_t),void* dummy)
{
    ...
    app_uart_inst_t *inst = CONTAINER_OF(hdl, app_uart_inst_t, inst);
    if(periph_semaphore_p(&inst->env.rx_lock)==false)
    {
        return PERIPH_BUSY;
    }
...
periph_err_t app_uart_read_cancel(periph_inst_handle_t hdl,uint32_t *remaining_length,void **dummy)
{
    app_uart_inst_t *inst = CONTAINER_OF(hdl, app_uart_inst_t, inst);
    periph_err_t error;
    uint32_t remaining_size;
    uart_universal_func.intr_op_func(inst,INTR_DISABLE);
    if(periph_semaphore_get_count(&inst->env.rx_lock))
    {
    ...
    }else
    {
        ...
        {
            periph_err_t dmac_error = app_dmac_transfer_cancel_wrapper(inst->env.dma_ch_rx,&remaining_size);
            if(dmac_error != PERIPH_NO_ERROR)
            {
                remaining_size = 0;
            }
            inst->env.dma_en_rx = false;
        ...
        {
            remaining_size = inst->env.rx.size;
    ...
    if(error == PERIPH_NO_ERROR)
    {
        ...
        if(remaining_length)
        {
            *remaining_length = remaining_size;
        }
    }
    return error;
}
...
periph_err_t app_uart_write(periph_inst_handle_t hdl,uint8_t *bufptr, uint32_t size, void (*callback) (void*,uint8_t),void* dummy)
{
    ...
    app_uart_inst_t *inst = CONTAINER_OF(hdl, app_uart_inst_t, inst);
    if(periph_semaphore_p(&inst->env.tx_lock)==false)
    {
        return PERIPH_BUSY;
    }
