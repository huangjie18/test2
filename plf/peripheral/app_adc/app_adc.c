...
void app_adc_init(periph_inst_handle_t hdl)
{
    ...
    //set thread lock
    periph_semaphore_init(&inst->env.adc_lock,1);
}
...
void app_adc_uninit(periph_inst_handle_t hdl)
{
    ...
    //set thread lock
    periph_semaphore_init(&inst->env.adc_lock,0);
}
...
periph_err_t app_adc_read_without_dma(periph_inst_handle_t hdl , uint8_t channel , uint16_t *value)
{
    ...
    reg_adc_t *reg = inst->reg;
    //thread lock
    if(periph_semaphore_p(&inst->env.adc_lock)==false)
    {
        return PERIPH_BUSY;
    }
    //read
    ...
    //thread unlock
    periph_semaphore_v(&inst->env.adc_lock);
    //adc value
