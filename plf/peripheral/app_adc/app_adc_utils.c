...
#define TEMP_SENSOR_ARRAY_SIZE 4
#if (defined(CFG_FREERTOS_SUPPORT)&&(CFG_FREERTOS_SUPPORT==1))
...
#if ADC_TRIM_MODE == ADC_TRIM_EXT_FLASH
...
void app_adc_util_init(void)
{
    ...
    #elif ADC_TRIM_MODE == ADC_TRIM_SIP_FLASH
    if(flash_read_security_reg(1, 0, 1, (uint8_t*)&adc_cp_RO) != PERIPH_NO_ERROR)
    {
        BX_ASSERT(0);
    }
    #else
    adc_cp_RO = 33; // default value for no trim IC under debug mode. Never run in production IC!!!
    #endif
    ...
    #else
    app_adc_RO_sim_3V9(); // calculate RO when battery = accurate 3.9V
    #endif    
...
#if ADC_GPADC_RO_TRIM
#if ADC_TRIM_MODE == ADC_TRIM_SIP_FLASH
...
#if (defined BX_TEMP_SENSOR) && (BX_TEMP_SENSOR == 1)  
static uint16_t temp_sensor[TEMP_SENSOR_ARRAY_SIZE] = {0};
void app_set_cur_temp(void)
{
    ...
    temp_sensor[TEMP_SENSOR_ARRAY_SIZE-1] = (uint16_t)(app_adc_tempSensor()/1000);
    if(temp_sensor[0] == 0) // first sample
    {
    ...
    #else
    temp_sensor[0] = (uint16_t)(app_adc_tempSensor()/1000);
    #endif
}
uint16_t app_get_cur_temp(void)
{
    ...
    return (uint16_t)(sum_temp/TEMP_SENSOR_ARRAY_SIZE);
}
