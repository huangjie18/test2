...
#define app_iic_set_int_unmask_stop_det(reg)        FIELD_WR(reg , IC_INTR_MASK , IIC_M_STOP_DET       , 1  )
#define app_iic_set_int_mask_stop_det(reg)          FIELD_WR(reg , IC_INTR_MASK , IIC_M_STOP_DET       , 0  )
...
static void iic_rx_complete(app_iic_inst_t *inst , uint8_t stat)
{
    ...
    app_iic_set_int_mask_stop_det(reg);
    ...
    //thread unlock
    periph_semaphore_v(&inst->env.iic_lock);
    // Call handler
    callback(dummy, stat);
}
...
static void iic_stop_det_isr(app_iic_inst_t *inst)
{
    if(inst->env.rw_mode == IIC_MODE_WRITE)
    {
...
void app_iic_isr(app_iic_inst_t *inst)
{
    ...
    if(irq_stat & (1<<IIC_INTR_STOP_DET)) {iic_stop_det_isr(inst); irq_stat &= ~(1<<IIC_INTR_STOP_DET);}
    //unexpected irq
    if(irq_stat)
    {
...
periph_err_t app_iic_read(periph_inst_handle_t hdl,uint8_t *bufptr, uint32_t size , uint16_t device_address, uint16_t mem_address , void (*callback) (void*,uint8_t),void* dummy)
{
    ...
    reg_iic_t *reg = inst->reg;
    //thread lock
    if(periph_semaphore_p(&inst->env.iic_lock)==false)
    {
        return PERIPH_BUSY;
    }
    ...
    app_iic_set_int_unmask_stop_det(reg);
    if(inst->param.use_dma)
    {
