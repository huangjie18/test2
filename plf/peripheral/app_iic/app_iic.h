...
struct iic_env_tag
{
    ...
    periph_semaphore_t iic_lock;
    /// dma channel 
    /// Make sure is atom action.Cannot use bit field. Especially there is lots of interrupt.
    /// If use bit field,and main write 'ch' in progress and isr write 'en',will cause the isr write 'en' has no effect.
    uint8_t dma_ch1_ch;
    uint8_t dma_ch1_en;
    uint8_t dma_ch2_ch;
    uint8_t dma_ch2_en;
