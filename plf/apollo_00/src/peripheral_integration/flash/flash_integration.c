...
SVCALL(SVCALL_FLASH_MULTI_READ_32BITS,periph_err_t,flash_multi_read_32bits_svcall(qspi_multi_read_32bits_param_t *param));
...
N_XIP_SECTION periph_err_t flash_multi_read_32bits_execution(qspi_multi_read_32bits_param_t *param)
{
    periph_err_t error;
    cache_disable();
    error = flash_multi_read_32bits_operation_start(param);
    cache_enable();
    return error;
}
...
{
    periph_err_t error;
    FLASH_EXECUTE(error,flash_multi_read_32bits,param);
    return error;
}
