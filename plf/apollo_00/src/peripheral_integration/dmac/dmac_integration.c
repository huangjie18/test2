...
N_XIP_SECTION static void dmac_clk_gate(app_dmac_inst_t *inst,clk_gate_operation_t op)
{
    if( op == SET_CLK )
    {
...
N_XIP_SECTION static void dmac_sys_stat(app_dmac_inst_t *inst,uint32_t sys_stat)
{
    switch(sys_stat)
    {
    case DMAC_INIT:
...
const periph_universal_func_set_t dmac_universal_func N_XIP_VARIABLE =
{
