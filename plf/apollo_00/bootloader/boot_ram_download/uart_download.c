...
#define FLASH_ONE_UART_SEND_PDU_SIZE    10240//10k      16384//16K 32768//32k //16384//16K   //4096//4k  
//download reply cmd
...
#define OP_CRC32_04C11DB7_RESULT        0x09
...
static void uart_tx_finish_ram_run(void);
 ...
 */
//crc32
static volatile uint32_t crc32_result=0;
static volatile uint32_t crc32_pc=0; 
static volatile bool crc32_flag=0;// 1 use crc32,0 no use
...
//Ram run parameter
static volatile bool ram_run_flag=0;//recive ram run
static volatile bool write_to_ram_run = 0;                   //write to RAM and run
 ...
 ****************************************************************************************
 */
static void uart_send_crc32_result(void)
{
    if(crc32_flag)
    {
        CRC32_XOR(crc32_result);

        if(crc32_pc == crc32_result)
        {
            uart_itf.tx_func(uart2pc[DOWNLOAD_ACK],sizeof(uart2pc[0]),uart_tx_finish_ram_run);//tell pc receive ok
        }
        else
        {
            uart_itf.tx_func(uart2pc[DOWNLOAD_NACK],sizeof(uart2pc[0]),uart_tx_finish_ram_run);//tell pc receive failed
        }
    }
}
...
void uart_rx_payload_finish()
{
    ...
    {   
        ...
        {
             bool no_ack=0;
             switch(uart_buff.header.cmd.op)
             {
                 case OP_PC_CMD ://                      0x01
                     {
                         if(uart_buff.header.length == 2)            //PC command
                         {
                     ...
                     break;
                 case OP_READ_FLASH_STATUS_REG ://        0x02
                     {
                         if(uart_buff.header.length == 2)   //read flash status reg   [0]idx  [1] reserved
                         {
                             uint8_t status=0;
                     ...
                     break;
                 case OP_READ_APOLLO_00_REG ://              0x03
                     {
                         if(uart_buff.header.length == 2)   //read apollo_00 reg
                         {
                     ...
                     break;
                 case OP_WRITE_APOLLO_00_REG ://            0x04
                     {
                         if(uart_buff.header.length == 4)   //write apollo_00 reg
                         {
                             uint32_t val;
                     ...
                     break;
                 case OP_WRITE_FLASH_STATUS_REG ://      0x05
                     {
                         if(uart_buff.header.length == 2)   //write flash status reg  [0]idx  [1] val
                         {
                             uint8_t status=0;
                             uint8_t reg_idx = uart_buff.data[0];
                     ...
                     break;
                 case OP_WRITE_W25_FLASH_STATUS_REG ://  0x06
                     {
                         if(uart_buff.header.length == 2)   //write w25 flash status reg [0]val_l  [1] val_h
                         {
                             uint16_t status=0;
                     ...
                     break;
                 case OP_WRITE_TO_RAM_AND_RUN ://        0x07
                     {
                         if(uart_buff.header.length == 2)
                         {
                             write_to_ram_run = 1;
                             uart_itf.tx_func(uart2pc[DOWNLOAD_ACK],sizeof(uart2pc[0]),uart_tx_finish);         //tell pc receive ok
                         }
                     ...
                     break;
                 case OP_WRITE_TO_RAM_COMPLETE ://        0x08
                     {
                         if(uart_buff.header.length == 2)
                         {
                             //start RAM RUN
                     ...
                     break;
                 case OP_CRC32_04C11DB7_RESULT ://       0x09
                     {
                         if(uart_buff.header.length == 4)
                         {
                             crc32_flag = 1;
                             crc32_result=CRC32_INIT_VAL;
                             crc32_pc = uart_buff.data[0] | (uart_buff.data[1] << 8) |uart_buff.data[2] << 16 | (uart_buff.data[3] << 24);
                ...
                case FLASH_WRITE:
                    {
                        ...
                        if(crc32_flag)
                        {
                            crc32_result = crc32_calc(crc32_result,uart_buff.data,uart_buff.header.length);
                            CRC32_XOR(crc32_result);
                        }
...
{
		uart_itf.tx_func(uart2pc[DOWNLOAD_NACK],sizeof(uart2pc[0]),uart_tx_finish);           //tell pc chip is ready to receive data  
...
static void uart_tx_finish_ram_run(void)
{
    if( write_to_ram_run && ram_run_flag )
    {
        if(crc32_flag)
        {
            if(crc32_pc == crc32_result)
